...
// ===================== REFRESH TUGAS =====================
function fetchTasks() {
    fetch(`${baseUrl}/api/tugas`, {
        method: "GET",
        credentials: "include", // penting untuk Sanctum (cookie-based)
        headers: {
        ...
        .then(async (response) => {
            ...
            if (data.success) {
                ...
                console.warn("Respons tidak berhasil:", data);
            }
        })
        .catch((error) => {
...
async function fetchCategories() {
    try {
        const response = await fetch(`${baseUrl}/api/kategori`, {
            method: "GET",
            credentials: "include", // penting untuk autentikasi Sanctum
            headers: {
                Accept: "application/json",
            },
        });
        ...
        if (data.success) {
            ...
            console.warn("Respons tidak sukses:", data);
        }
    } catch (error) {
...
fetch(`${baseUrl}/api/user`, {
    ...
    .then(async (response) => {
        ...
        const data = await response.json();
        const userName = data.name || "Kawan";
...
class StreakManager {
    ...
    async fetchStreakData() {
        try {
            const response = await fetch(`${baseUrl}/api/streaks/summary`, {
                method: "GET",
                credentials: "include",
                headers: {
                    Accept: "application/json",
            ...
            const data = await response.json();
            const { current_streak } = data.data ?? 0;
            const test = data.data.current_streak;
            await this.updateStreak("0"); // BUAT TES STREAK DISINI
        } catch (error) {
            console.error(
                "Terjadi kesalahan saat mengambil data streak:",
                error
            );
            await this.updateStreak(0); // Default saat error
        }
    }
    ...
    async updateStreak(newStreak) {
        ...
        // this.elements.streakContent.textContent = newStreak;
    ...
    updateStreakTitle() {
        ...
        if (this.elements.streakTitle) {
            ...
            this.elements.streakTitle.style.animation =
    ...
    updateStreakTheme() {
        ...
        themes.forEach((theme) => {
            this.elements.streakContent.classList.remove(
                `streak-theme-${theme}`
            );
        });
        ...
        this.elements.streakContent.classList.add(
            `streak-theme-${currentTheme}`
        );
    ...
    async animateStreakCount() {
        ...
        return new Promise((resolve) => {
            let currentCount = 0;
            const duration = 1000;
            const steps = Math.min(targetCount, 50);
            ...
            const updateCount = () => {
                ...
                if (displayCount >= targetCount) {
                    this.elements.streakCount.style.animation =
    ...
    createSparkles() {
        ...
        for (let i = 0; i < sparkleCount; i++) {
            ...
            sparkle.className =
                Math.random() > 0.5 ? "sparkle star" : "sparkle";
    ...
    startAnimations() {
        ...
        setTimeout(() => {
            if (this.elements.streakContent) {
                this.elements.streakContent.style.animation =
                    "count-up 0.8s ease-out";
            }
        }, 200);
    }
...
class TaskFormManager {
        ...
        if (startTimeValue) {
            ...
            if (this.elements.endTime) {
                ...
                if (
                    ...
                    this.elements.endTime.value <= startTimeValue
                ) {
                    this.elements.endTime.value = "";
                }
            }
    ...
    selectCategory(categoryId, source = "chip") {
        ...
        if (source === "chip") {
            ...
            const selectedChip = this.elements.categoryContainer.querySelector(
                `[data-id="${categoryId}"]`
            );
            if (selectedChip) {
            ...
            const allChips =
                this.elements.categoryContainer.querySelectorAll(
                    ".category-btn"
                );
    ...
    clearCategorySelections() {
        ...
        const allChips =
            this.elements.categoryContainer.querySelectorAll(".category-btn");
        allChips.forEach((chip) => chip.classList.remove("active"));
    ...
    validateForm() {
        ...
        // Validate title
        const title = this.elements.taskTitle
        ...
        // Validate start time
        const startTime = this.elements.startTime
            ? this.elements.startTime.value
            : "";
        if (!startTime) {
            errors.push("Waktu mulai harus diisi");
            if (this.elements.startTime)
                this.elements.startTime.classList.add("error");
        } else {
            const now = new Date();
            ...
            if (startDateTime < now) {
                ...
                if (this.elements.startTime)
                    this.elements.startTime.classList.remove("error");
            }
        }

        // Validate end time (if filled)
        const endTime = this.elements.endTime
            ? this.elements.endTime.value
            : "";
        ...
        if (!this.selectedCategoryId) {
            ...
            if (this.elements.categorySelect)
                this.elements.categorySelect.classList.add("error");
        } else {
            if (this.elements.categorySelect)
                this.elements.categorySelect.classList.remove("error");
        }
    ...
    async handleSubmit() {
        ...
        try {
            const taskData = {
                ...
                deskripsi: this.elements.taskDescription
                    ? this.elements.taskDescription.value.trim() ||
                    "Tidak ada deskripsi"
                    : "Tidak ada deskripsi",
                waktu_mulai: formatDatetime(this.elements.startTime.value),
                waktu_selesai:
                    this.elements.endTime && this.elements.endTime.value
                        ? formatDatetime(this.elements.endTime.value)
                        : null,
                is_completed: false,
                completed_at: null,
            };

            const response = await fetch(`${baseUrl}/api/tugas`, {
                method: "POST",
                headers: {
        ...
        if (incompleteTasks.length > 0) {
            incompleteTasks.forEach((task) => {
                ...
                const waktuSelesai = task.waktu_selesai
                    ? new Date(task.waktu_selesai).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                      })
                    : "";

                const waktuDitampilkan = waktuSelesai
...
todoListElement.addEventListener("click", (e) => {
    ...
    if (checkbox) {
        ...
        const isCompleted = pathElement.style.opacity === "1";
        pathElement.style.opacity = isCompleted ? "0" : "1";

        fetch(`${baseUrl}/api/tugas/${taskId}/toggle-complete`, {
            method: "POST",
            credentials: "include",
            headers: {
                Accept: "application/json",
            ...
            .then(async (response) => {
                ...
                if (data.success) {
                    ...
                    // 🚀 Tampilkan pesan jika sudah kosong
                    if (todoListElement.children.length === 0) {
                        todoListElement.innerHTML = `<p>Wah, Tidak ada tugas yang tersisa!</p>`;
                    }

                    // Refresh streak jika ada streakManager
                    if (typeof window.streakManager !== "undefined") {
                        window.streakManager.refresh();
                    }
                }
            ...
            .catch((error) => {
                ...
                pathElement.style.opacity = isCompleted ? "1" : "0";
            });
    }
});
...
async function renderCategoryButtons() {
    try {
        ...
        if (result.success && result.data.length > 0) {
            ...
            // Tampilkan maksimal 3 kategori sebagai tombol
            ...
            // Kategori sisanya masuk ke dalam dropdown
            if (result.data.length > 3) {
                result.data.slice(3).forEach((item) => {
            ...
            // Set default kategori untuk TaskFormManager jika tersedia
            if (window.taskFormManager && result.data.length > 0) {
                window.taskFormManager.selectedCategoryId = result.data[0].id;
            }
        }
    } catch (error) {
